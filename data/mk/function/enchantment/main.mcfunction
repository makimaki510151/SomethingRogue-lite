...
execute if data entity @s equipment.offhand.components."minecraft:custom_data"{"enchantment_id":"armor_toughness_1_2"} if data entity @s SelectedItem.components."minecraft:custom_data".enchantment.contents_list{gravity:true} run return run tellraw @s [{text:"このアイテムには既に同じ効果の付与が存在します。"}]
...
execute if data entity @s SelectedItem.components."minecraft:enchantments" run return run tellraw @s [{text:"結晶によるエンチャントがすでに付いています"}]
...
execute if data entity @s equipment.offhand.components."minecraft:custom_data"{"enchantment_id":"armor_toughness_1_2"} run function mk:enchantment/item/armor_toughness_1_2
...
execute if data entity @s equipment.offhand.components."minecraft:custom_data"{"enchantment_id":"gravitational_convergence"} run function mk:enchantment/item/gravitational_convergence

execute store result score temp1 system run data get entity @s equipment.offhand.count 1
scoreboard players remove temp1 system 1
execute if score temp1 system matches 0 run item replace entity @s weapon.offhand with air
execute if score temp1 system matches 1.. run item modify entity @s weapon.offhand mk:count_temp1
